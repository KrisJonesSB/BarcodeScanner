...
public protocol BarcodeScannerCodeDelegate: class {
  ...
  func customButtonPressed(_ controller: BarcodeScannerController)
}
...
open class BarcodeScannerController: UIViewController {
  ...
  /// Button to change torch mode.
  lazy var customButton: UIButton = { [unowned self] in
    let button = UIButton(type: .custom)
    button.setTitle( CustomButton.text , for: UIControlState.normal )
    button.tintColor = CustomButton.color
    button.addTarget(self, action: #selector(customButtonTap), for: .touchUpInside)
  ...
  open override func viewDidLoad() {
    ...
    [infoView, headerView, settingsButton, flashButton, customButton, focusView].forEach {
      view.addSubview($0)
